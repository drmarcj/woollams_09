  ...
  for(i=0;i<500;i++)
    {
      ...
      euclid(phonOut->outputs[SAMPLES-1],euclid_output);

      printf("%s\t",euclid_output);
      
      for(j=0;j<phonOut->numUnits;j++)
	target[j]=get_value(ex->targets,phonOut->index,SAMPLES-1,j); 
      
      euclid(target,target_output);
      printf("%s\t(%.2f)\t",target_output,sse);
